...
export class MenuComponent implements OnInit, AfterViewInit {
  ...
  private convertMenuToSideMenu() {
    ...
    this.categoryMenus.forEach((f, i) => {
      const fData = {
        ...f,
        ...{parentId: null, routerLink: `/products-list/${f.slug}`, hasSubMenu: f.hasChild && f.hasChild.length > 0, depth: 1, hasChild: []}
      };
      deep1.push(fData);
      if (f.hasChild && f.hasChild.length > 0) {
        f.hasChild.forEach(g => {
          const gData = {
            ...
            ...{
              parentId: f.id,
              routerLink: `/products-list/${f.slug}/${g.slug}`,
          ...
          if (g.hasChild && g.hasChild.length > 0) {
            g.hasChild.forEach(h => {
              const hData = {
                ...h,
                ...{parentId: g.id, routerLink: `/products-list/${f.slug}/${g.slug}/${h.slug}`, hasSubMenu: false, depth: 3, hasChild: []}
              };
              deep3.push(hData);
            });
          }
